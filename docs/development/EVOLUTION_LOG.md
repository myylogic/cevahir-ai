...
## 2026-09-21 — Academic neural, attention, Transformer and training expansion

Expanded chapters 3-6 with mathematical definitions and derivations, topic-to-file/method maps, original-paper citations, dated industry comparisons and 22 solved exercises. Seven CPU example groups exercise actual Cevahir embedding, attention, cache, FFN, normalization, RoPE and the small neural core; hand-selected pedagogical examples remain explicitly separate. Forward and backward preserve parameters, while the isolated in-memory SGD step changes them. The book now has 12 chapters, 98 second-level headings and 50 third-level headings; these are structural counts, not a claim of complete conceptual coverage.

99 selected attention/cache/lower-layer/MoE/training contract tests passed with six existing deprecation warnings. Source review distinguishes accepted dynamic RoPE configuration from implemented scaling, top-1 router gradient limitations, model names from actual code, and causal behavior from the Encoder class name. The academic reading guide states prerequisites, attribution, evidence scope and remaining editorial coverage. Runtime code, trained assets and original research records were not changed. A second versioned collection archive is prepared; its confirmed DOI is recorded separately after publication.

Publication confirmation: research-2026.09.21-v2 at 0a13c2889c7005aed850071f9964cded67997f1f was archived as DOI 10.5281/zenodo.22874751, version 2026.09.21.2. All 841 archived files match the Git tag byte for byte; the DOI resolves to the expected public record and author/version metadata match. Prior archive and evidence remain intact. DOI bookkeeping is appended on main after publication without moving the tag.


## 2026-09-21 — Checkpoint, generation, memory and tool book expansion

Expanded chapters 7–9 with checkpoint identity and optimizer continuation, temperature/filter mathematics, beam and cache limits, retrieval scoring and scope, candidate selection and actual tool execution. Added 15 solved exercises, 18 bibliography entries and ten executed CPU example groups. The chapter files now contain 12 chapters, 115 H2 topics and 64 H3 subtopics; whitespace word count including markup is 29,547. These counts do not claim complete coverage or external peer review.

Source review reproduced a real calculator inference bug at 8fb26e3: `2+3*4` was truncated to `2+3` and returned the string `5`. Under the user's authorization to fix discovered code errors, inference now preserves one complete supported arithmetic span, validates it with AST without evaluation, and declines ambiguous/unsupported input instead of inventing addition. Compound, parenthesized, decimal, signed and scientific-notation examples exercise the actual context handler. Original failing output is preserved as historical evidence; the updated runtime example returns `14` for both inferred and explicit full expressions. This remains a limited expression recognizer, not general natural-language mathematics.

70 selected generation, agent, checkpoint and runtime tests passed in 20.99 seconds. Lifecycle and runtime walkthroughs each passed five groups. The optimizer-resume experiment reproduces the next update with restored moments but diverges by about 0.00489928 when optimizer history is reset. Fixed-logit filter examples and hand-assigned retrieval vectors are clearly distinguished from trained-model quality. Existing research reports and negative results remain intact. Checksum and alias-atomicity limitations are documented; this tranche does not redesign checkpoint storage.

Publication confirmation: research-2026.09.21-v3 at 3c8a52c4d4fffff2ebd533dd55bccdbce4e85434 is archived as DOI 10.5281/zenodo.22875504, version 2026.09.21.3. All 854 archived paths and byte contents match the release tag; author/version metadata and public DOI resolution were verified. GitHub book-source CI passed for the tagged commit. Prior versions and findings remain unchanged.
