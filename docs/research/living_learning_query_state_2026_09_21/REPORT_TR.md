...
Türetim ve karşı örneklerin ayrıntılı ön çalışma kaydı: [matematiksel not](../../publications/working-notes/future-state-review.md). Bu iç araştırma değerlendirmesi dış hakemlik değildir.
...
Bu komut kayıtlı sonuçların üzerine yazmadan tam taramayı tekrarlar. Yayın paketi ayrıca [koruma ve doğrulama kaydını](../../publications/evidence/validation.json) taşır. Araştırma ve metin hazırlığında yapay zekâ desteği kullanılmıştır; proje ve araştırma yönü Muhammed Yasin Yılmaz'a aittir.
