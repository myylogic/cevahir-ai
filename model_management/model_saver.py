...
# Cevahir versiyon sabiti — checkpoint meta verisine yazılır
_CEVAHIR_CKPT_VERSION = "4.1"
_CEVAHIR_FORMAT_VERSION = 2   # Checkpoint format numarası (kırıcı değişikliklerde artar)
...
def _compute_sha256(data: bytes) -> str:
    """SHA-256 hash'ini hex string olarak döndürür."""
    return hashlib.sha256(data).hexdigest()


def _compute_file_sha256(path: str) -> str:
    """Disk üzerindeki dosyanın SHA-256 hash'ini hesaplar (streaming, büyük dosya dostu)."""
    sha = hashlib.sha256()
...
def _prune_old_checkpoints(
    ...
    keep_last_n: int,
    *,
    prefer_best_val_loss: bool = True,
) -> None:
    """
    Belirli bir önekle başlayan eski checkpointleri akıllıca budar.

    Strateji (prefer_best_val_loss=True):
      1. Her checkpoint'in metadata dosyasındaki val_loss'a bak.
      2. En iyi val_loss'a sahip checkpoint KESİNLİKLE korunur.
      3. Geri kalanlar arasından en yeniler korunur, eskiler silinir.

    Metadata yoksa basit dosya adı sıralamasına (eski davranış) düşer.
    """
    try:
        files = sorted(
            [f for f in os.listdir(save_dir) if f.startswith(pattern_prefix) and f.endswith(".pth")]
        )
        if keep_last_n <= 0 or len(files) <= keep_last_n:
            return

        if prefer_best_val_loss:
            # Her dosya için val_loss almaya çalış
            ...
            for fn in files:
                ...
                try:
                    ...
                    if isinstance(ckpt, dict):
                        meta = ckpt.get("metadata") or ckpt.get("additional_info") or {}
                        if isinstance(meta, dict):
                            val_loss = float(meta.get("val_loss", float("inf")))
                except Exception:
                    pass
                scored.append((val_loss, fn))

            # En iyi val_loss'a sahip dosyayı bul
            best_fn = min(scored, key=lambda x: x[0])[1]

            # Tarih sıralamasına göre (en eskiden yeniye) sil, ama best'i koru
            candidates_to_remove = [fn for _, fn in scored if fn != best_fn]
            # keep_last_n - 1: best zaten korunuyor, geriye kalan slotu son N-1 için
            to_keep_count = max(0, keep_last_n - 1)
            to_remove = candidates_to_remove[:-to_keep_count] if to_keep_count > 0 else candidates_to_remove
        else:
        ...
        for fn in to_remove:
            try:
                ...
                saver_logger.debug(f"Eski checkpoint silindi: {fn}")
            except Exception as e:
    ...
    def save_checkpoint(
        ...
        *,
        ...
        try:
            ...
            # Parametre sayısı (meta veri için)
            ...
            ckpt = {
                ...
                "metadata": {
                    **(metadata or {}),
                    # ── Cevahir Versiyon Bilgisi ──────────────────────────────
                    "cevahir_version": _CEVAHIR_CKPT_VERSION,
                    "checkpoint_format": _CEVAHIR_FORMAT_VERSION,
            ...
            # ── Atomik Kaydet ─────────────────────────────────────────────────
            # BytesIO'ya serialize et, SHA-256 hesapla, sonra diske yaz
            buf = io.BytesIO()
            torch.save(ckpt, buf)
            raw_bytes = buf.getvalue()

            # SHA-256 checksum'ı checkpoint meta'sına gömebiliriz (iç doğrulama için)
            ckpt_sha256 = _compute_sha256(raw_bytes)
            ckpt["metadata"]["sha256"] = ckpt_sha256

            # Checksum dahil son hali tekrar serialize et
            buf2 = io.BytesIO()
            torch.save(ckpt, buf2)
            _atomic_write_bytes(path, buf2.getvalue())

            # SHA-256 sidecar dosyası (.sha256) — dışarıdan hızlı doğrulama için
            sha_path = path + ".sha256"
            try:
                _atomic_write_bytes(sha_path, _compute_file_sha256(path).encode("utf-8"))
            except Exception as sha_exc:
                saver_logger.warning(f"SHA-256 sidecar yazılamadı: {sha_exc}")

            saver_logger.info(
                f"Checkpoint kaydedildi: {path} "
                f"(epoch={epoch}, params={total_params/1e6:.1f}M, sha256={ckpt_sha256[:12]}...)"
            )
