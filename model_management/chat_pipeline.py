...
class ChatPipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # train.py ile aynı mimari olmalı (checkpoint yükleme); güncel: embed_dim=512, num_heads=8, num_layers=8
        model_cfg.setdefault("embed_dim", self.config.get("embed_dim", 512))
        model_cfg.setdefault("seq_proj_dim", self.config.get("seq_proj_dim", 512))
        model_cfg.setdefault("num_heads", self.config.get("num_heads", 8))
...
def main() -> None:
    ...
    config: Dict[str, Any] = {
        ...
        # Entropy collapse düzeltmesi: training entropy=0.5 (max=11.0) → dağılım aşırı peaked
        # Temperature yüksek tutulmazsa model hep tek token (nokta vb.) üretiyor
        "temperature": 2.0,    # Entropy=0.5 → peaked dağılımı yaymak için yüksek temperature şart
        "top_k": 80,           # Geniş havuz; collapsed model için çeşitlilik gerekli
        "top_p": 0.98,         # Neredeyse tüm dağılım; kısıtlama yok
        "repetition_penalty": 1.05,  # Minimum; zaten entropy düşük, fazla ceza anlamsız
